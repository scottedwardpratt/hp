...
void CAcceptance_STAR::CalcAcceptance(bool &accept,double &efficiency,CHBPart *part,int CENTRALITY){
...
void CAcceptance_STAR::CalcAcceptanceNoID(bool &accept,double &efficiency,CHBPart *part,int CENTRALITY){
	double eta,pt,pmag,gammav,phi,*p=part->p,y=part->y;
	double dca[4];
	int pid=part->resinfo->code,starpid;
	if(abs(pid)==211) starpid=1;
...
double phi,int cen,bool &accept,double &eff){
