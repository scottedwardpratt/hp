...
double CResInfo::GenerateThermalMass(double maxweight, double T){
	...
	if(decay || maxweight<0.0){
		...
		double k2mr = cyl_bessel_k(2,mass/T); // K2 for resmass
		kr=pow(mass*mass-m1*m1-m2*m2,2) - (4*m1*m1*m2*m2);
		kr = (1/(2*mass))*sqrt(kr); // k at resonant mass
		int i = 0; // for use in while loop
		while(i==0){
			double r1 = randy->ran(); // get random numbers
			double r2 = randy->ran(); // between [0, 1]
			...
			double rho=(2.0/(width*PI))*(0.25*gamma*gamma)/((0.25*gamma*gamma)+(mass-m)*(mass-m));
			double lor = (width/(2*PI))/(pow(width/2,2.0) + pow(mass-m,2.0));
			double k2 = cyl_bessel_k(2,(m/T)); // K2 value
...
double &dedt,double &maxweight){
	...
	for(E=(m1+m2+0.5*dE);E<2.0*resmass;E+=dE){
		...
		n0=cyl_bessel_k(2,E/T)*E*E*T/(2*PI*PI*pow(HBARC,3.0));
		resn0=cyl_bessel_k(2,resmass/T)*resmass*resmass*T/(2*PI*PI*pow(HBARC,3.0));
...
double &nh,vector<double> &density,vector<double> &maxweight,arma::mat &chi){
	CResInfo *resinfoptr;
	CResInfoMap::iterator rpos;
...
double ChiOmega(double vOmega,double T){
	double chiomega;
	return chiomega;
}
