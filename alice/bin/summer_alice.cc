...
int main(int argc,char *argv[]){
	...
	double normchecky=0.0,normcheckphi=0.0;
	...
	for(itype=0;itype<NTYPES;itype++){
		...
		for(iy=0;iy<NYBINS;iy++){
			...
			printf("%4.2f %g\n",(iy+0.5)*DY,Bysum[iy]);
			normchecky+=(YMAX/double(NYBINS))*Bysum[iy];
		}
		fclose(fptr_write);
		...
		for(iphi=0;iphi<NPHIBINS;iphi++){
			...
			printf("%4.2f %g\n",-180.0+(iphi+0.5)*DPHI,Bphisum[iphi]);
			normcheckphi+=(PI/double(NPHIBINS))*Bphisum[iphi];
		}
		fclose(fptr_write);
		
	}
	printf("normchecky=%g, normcheckphi=%g\n",normchecky,normcheckphi);
	return 0;
}
